...
--EXERCICIO#16
select min(PRECO_TABELA) as preco_mais_baixo, max(PRECO_TABELA) as preco_mais_alto, round(avg(PRECO_TABELA/2)) as tabela
...
select titulo, preco_tabela,unidades_vendidas "livros vendidos", (unidades_vendidas*preco_tabela) as receitas_esperadas, (UNIDADES_VENDIDAS * preco_tabela * 1.05) as receita_efetiva
from livros
/*
...
where livros,autores
and (autores.nome like
group by genero
order by genero asc;
